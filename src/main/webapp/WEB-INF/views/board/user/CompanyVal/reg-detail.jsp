...
<nav class="container">
	<img src="/resources/image/community1.png" width="1000px" id="re"></nav>
    	...
    	<div class="div2">
        	...
        	<h6>조회수 
        		<c:out value="${board.views }"/> 
        			<a href="javascript:goPost('Like')" 
        			   style="text-decoration: none; color: rgb(255, 149, 206);">
           		   	   좋아요
           		   	</a> 
        	...
        	<form id="operForm" action="/board/user/CompanyVal/modify" method="get">
...
$(document).ready(function () {	
	...
	function showList(page) {
		console.log("showList page: "+page);
		                                   ...
		                                   //저장된 페이지 값이 없으면 1페이지
		replyService.companyVal_GetList({bno:bnoValue, page:page||1},
		...
		}); //end replyService.companyVal_GetList
	...
	$(".addReply").on("click", function (e) {
		...
		"</sec:authorize>"
		replyService.companyVal_Add(reply, function (result) {
			alert("등록되었습니다.");
	...
	$(".chat-list").on("click", "li", function (e) {
		...
		replyService.companyVal_Get(rno, function (reply) {
	...
	modalModBtn.on("click", function(e){
		...
		replyService.companyVal_Update(reply, function(result){
			alert("댓글이 수정되었습니다.");
			$("#myModal").modal("hide");
			showList(pageNum);
		});//end replyService.companyVal_Update
	...
	modalRemoveBtn.on("click", function(e){
		...
		replyService.companyVal_Remove(rno, function(deleteResult){
			alert("댓글이 삭제되었습니다.");
			$("#myModal").modal("hide");
			showList(pageNum);
		}); //end replyService.companyVal_Remove
...
function goPost(Like){
	...
	obj.setAttribute('value', 'CompanyVal');
	...
	p2.setAttribute('action',"/board/user/CompanyVal/reg-detail2?bno=<c:out value="${board.bno }"/>");
...
$(document).ready(function () {
	...
	$(".btn-reg-detail").on("click", "button", function (e) {
		...
		if(operation === "modify"){
			...
			operForm.attr("action", "/board/user/CompanyVal/modify");
			
		}else if(operation === "community"){
			$("button[data-oper='community']").unbind('click');
			operForm.find("#bno").remove();
			operForm.attr("action", "/board/user/CompanyVal/community");
			
		}else if(operation === "remove"){
			if(confirm("정말 삭제하시겠습니까?") == true){
			$("button[data-oper='remove']").unbind('click');
			operForm.attr("action", "/board/user/CompanyVal/remove").attr("method", "post");
			}else{
				return false;
			}
		}else if(operation === "reg-detail+"){
			operForm.attr("action", "/board/user/CompanyVal/reg-detail?bno=<c:out value="${board.bno }"/>").attr("method","post");
		}
