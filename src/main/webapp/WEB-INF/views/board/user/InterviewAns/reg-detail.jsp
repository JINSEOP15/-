...
<sec:authorize access="isAuthenticated()">
    ...
    <div class="div">
    	<div class="div2">
        	...
        	<form id="operForm" action="/board/user/InterviewAns/modify" method="get">
...
$(document).ready(function () {	
	...
	function showList(page) {
		console.log("showList page: "+page);
		                                   ...
		                                   //저장된 페이지 값이 없으면 1페이지
		replyService.interviewAns_GetList({bno:bnoValue, page:page||1},
		...
		}); //end replyService.interviewAns_GetList
	...
	$(".addReply").on("click", function (e) {
		...
		"</sec:authorize>"
		replyService.interviewAns_Add(reply, function (result) {
			alert("등록되었습니다.");
		...
		}); //end replyService.interviewAns_Add
	...
	$(".chat-list").on("click", "li", function (e) {
		...
		replyService.interviewAns_Get(rno, function (reply) {
	      ...
	      "</sec:authorize>"
		}); //end replyService.interviewAns_Get
	...
	modalModBtn.on("click", function(e){
		...
		replyService.interviewAns_Update(reply, function(result){
			alert("댓글이 수정되었습니다.");
			$("#myModal").modal("hide");
			showList(pageNum);
		});//end replyService.interviewAns_Update
	...
	modalRemoveBtn.on("click", function(e){
		...
		replyService.interviewAns_Remove(rno, function(deleteResult){
			alert("댓글이 삭제되었습니다.");
			$("#myModal").modal("hide");
			showList(pageNum);
		}); //end replyService.interviewAns_Remove
...
function goPost(Like){
	...
	p2.setAttribute('action',"/board/user/InterviewAns/reg-detail2?bno=<c:out value="${board.bno }"/>");
...
$(document).ready(function () {
	...
	$(".btn-reg-detail").on("click", "button", function (e) {
		...
		if(operation === "modify"){
			...
			operForm.attr("action", "/board/user/InterviewAns/modify");
			
		}else if(operation === "community"){
			$("button[data-oper='community']").unbind('click');
			operForm.find("#bno").remove();
			operForm.attr("action", "/board/user/InterviewAns/community");
			
		}else if(operation === "remove"){
			if(confirm("정말 삭제하시겠습니까?") == true){
				$("button[data-oper='remove']").unbind('click');
				operForm.attr("action", "/board/user/InterviewAns/remove").attr("method", "post");
				}else{
					return false;
				}
		}else if(operation === "reg-detail+"){
			operForm.attr("action", "/board/user/InterviewAns/reg-detail?bno=<c:out value="${board.bno }"/>").attr("method","post");
		}
